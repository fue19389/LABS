...
rsttmr0 macro
    ...
    bcf ((INTCON) and 07Fh), 2 ; limpiar la bandera siempre
    endm
...
rsttmr1 macro
    ...
    movwf TMR1L
    bcf ((PIR1) and 07Fh), 0 ; limpiar la bandera siempre CAMBIAR A T1
    endm
...
isr:
    btfsc ((INTCON) and 07Fh), 2 ; Bandera del timer1
    call int_t0
    btfsc ((PIR1) and 07Fh), 0 ; Bandera del timer0
    call int_t1
    btfsc ((PIR1) and 07Fh), 1 ; Bandera del timer2
    call int_t2
    btfsc ((INTCON) and 07Fh), 0 ;Subrutina de interrupción pullups
    call int_iocb
...
int_iocb: ;Subrutina de interrupción para el puerto B
    ...
    bcf ((INTCON) and 07Fh), 0 ;Clear de la bandera del puerto B
    return
...
cfg_clk:
    ...
    bcf ((OSCCON) and 07Fh), 6 ; Seteo de frecuencia a 500kHz (011)
    bsf ((OSCCON) and 07Fh), 5 ; "    " "           "
    bsf ((OSCCON) and 07Fh), 4 ; "    " "           "
    bsf ((OSCCON) and 07Fh), 0 ; Selección de reloj interno
    return ; Regresar al main

cfg_inte:
    banksel INTCON
    bsf ((INTCON) and 07Fh), 7 ; Enable del interrupt global
    bsf ((INTCON) and 07Fh), 6 ; Enable interrupt periferico
    bsf ((INTCON) and 07Fh), 5 ; Enable global del t0
    bsf ((INTCON) and 07Fh), 3 ; Enable del change interrupt
    bcf ((INTCON) and 07Fh), 0

    banksel PIE1
    bsf ((PIE1) and 07Fh), 0 ; Enable dentro de perifericos t1
    bsf ((PIE1) and 07Fh), 1 ; Enable dentro de perifericos t2
...
cfg_iocb:
    ...
    bsf IOCB, 1 ; " 
    bsf IOCB, 2
    bcf ((INTCON) and 07Fh), 0 ; Bandera de interrupt b
    return

cfg_t0:
    banksel OPTION_REG ; Selección del banco a trabajar
    bcf ((OPTION_REG) and 07Fh), 5 ; Selecciónd el reloj a utilizar en tmr0 (interno)
    bcf ((OPTION_REG) and 07Fh), 3 ; Selección del preescaler para el TMR0 y no WDT
    bsf ((OPTION_REG) and 07Fh), 2 ; Selección del preescaler a 1:256 (111 en ((OPTION_REG) and 07Fh), 2 -0)
...
cfg_t1:
    ...
    bcf ((T1CON) and 07Fh), 1 ; Selección del reloj a utilizar en tmr1 (interno)
    bcf ((T1CON) and 07Fh), 5 ; Prescaler a 1:2
    bsf ((T1CON) and 07Fh), 4
    bcf ((T1CON) and 07Fh), 3 ; Oscilador de bajo voltaje apagado
    bsf ((T1CON) and 07Fh), 0 ; Enable del TMR1
    bcf ((T1CON) and 07Fh), 6 ; Gate option apagado
    banksel PORTA ; regresar al banco del puerto A
...
cfg_t2:
    banksel T2CON
    bsf ((T2CON) and 07Fh), 6 ; Postscaler a 1:16
    bsf ((T2CON) and 07Fh), 5
    bsf ((T2CON) and 07Fh), 4
    bsf ((T2CON) and 07Fh), 3

    bcf ((T2CON) and 07Fh), 2 ; Enable del TMR2

    bsf ((T2CON) and 07Fh), 1 ; prescaler en 00: es decir 1:16
